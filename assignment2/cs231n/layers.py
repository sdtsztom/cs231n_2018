...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        out = gamma * x_ + beta
        cache = (x , x_, gamma , beta, mean , var , eps)
...
def batchnorm_backward(dout, cache):
    ...
    x, x_ , gamma , beta , mean , var , eps = cache
    N,D = x.shape
    dx_ = dout * gamma
    dvar = np.sum(dx_ * (x-mean) , axis=0) * (-0.5) * (var+eps) ** (-3/2.)
    dmean = np.sum(dx_ / -np.sqrt(var + eps) , axis=0) + dvar * (np.sum(-2 * (x-mean) , axis=0)) / N
    dx = dx_ / np.sqrt(var + eps) + dvar * 2 * (x-mean) / N + dmean / N
...
def layernorm_backward(dout, cache):
    ...
    N,D = x.shape
    dx_ = dout * gamma
    dvar = np.sum(dx_ * (x-mean),axis=1,keepdims=True) * (-0.5) * (var+eps) ** (-3/2.)
    dmean = np.sum(dx_ / -np.sqrt(var + eps),axis=1,keepdims=True) + dvar * (np.sum(-2 * (x-mean),axis=1,keepdims=True)) / D
    dx = dx_ / np.sqrt(var + eps) + dvar * 2 * (x-mean) / D + dmean / D
    dgamma = np.sum(dout * x_ , axis=0)
    dbeta = np.sum(dout , axis=0)
