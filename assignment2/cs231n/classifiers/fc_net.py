...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dH1, dW2, db2 = affine_backward(dH2, cache2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers+1):
            # affine
            hi , cache['af%d'%(i)] = affine_forward(hi , self.params['W%d'%(i)] , self.params['b%d'%(i)] )
            ...
            if i<self.num_layers:
                ...
                # dropout
                if self.use_dropout:
                    hi, cache['dp%d'%(i)] = dropout_forward(hi, self.dropout_param)
        ...
        for i in range(self.num_layers,0,-1):
            ...
            if i<self.num_layers:
                # dropout
                if self.use_dropout:
                    dhi=dropout_backward(dhi , cache['dp%d'%(i)])
